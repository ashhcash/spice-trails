...
<main id="top">
    ...
    <section class="blog-list-hero" aria-labelledby="blog-list-title">
        <div>
            ...
            <p>Browse the latest recipes by category, or search for a dish, place, ingredient, or memory.</p>
        </div>
    </section>
    ...
    <section class="blog-list-layout" id="stories">
        <aside class="blog-filter-sidebar" aria-label="Blog filters">
            <section class="panel blog-search-panel">
                ...
                <form action="<?= base_url('recipes') ?>" method="get" class="blog-search-form">
                    <?php if ($activeCategory !== ''): ?>
                        <input type="hidden" name="category" value="<?= esc($activeCategory) ?>">
                    <?php endif; ?>
                    <label class="visually-hidden" for="blogSearch">Search Recipes</label>
            ...
            <section class="panel">
                ...
                <div class="blog-category-list">
                    <a class="<?= $activeCategory === '' ? 'active' : '' ?>" href="<?= base_url('recipes') ?>">
                        All Recipes
                    </a>
                    <?php foreach ($categories as $category): ?>
                        ...
                        <?php if ($categoryName !== ''): ?>
                            <a class="<?= $activeCategory === $categoryName ? 'active' : '' ?>"
                                href="<?= base_url('recipes') . '?' . http_build_query(['category' => $categoryName, 'search' => $search]) ?>">
                                <?= esc($categoryName) ?>
                            </a>
                        <?php endif; ?>
                    <?php endforeach; ?>
        ...
        <div class="blog-list-content">
            <div class="section-heading">
                ...
                <h2><?= $activeCategory !== '' ? esc($activeCategory) : 'All Recipe Stories' ?></h2>
            </div>
            ...
            <?php if ($search !== '' || $activeCategory !== ''): ?>
                <div class="active-blog-filters">
                    ...
                    <?php endif; ?>
                    <a href="<?= base_url('recipes') ?>">Clear filters</a>
                </div>
            <?php endif; ?>
            ...
            <div class="blog-card-grid row g-2 pt-4">
                <?php if (!empty($recipes)): ?>
                    <?php foreach ($recipes as $recipe): ?>
                        ...
                        $description = trim(strip_tags($recipe['description'] ?? ''));
                        $date = $recipe['date'] ?? '';
                        $timestamp = $date !== '' ? strtotime($date) : false;
                        $image = $recipe['image'] ?? '';
                        $imageUrl = $image !== ''
                            ? base_url('public/assets/recipeuploads/' . basename($image))
                        ...
                        <article class="blog-card col-lg-3 col-md-4 col-sm-12">
                            <div class="outline">
                                ...
                                <div class="blog-card-body">
                                    ...
                                    <a class="read-link" href="<?= base_url('recipes/' . $recipe['slug']) ?>">View Recipe</a>
                                </div>
                            </div>
                        </article>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="empty-state">No recipess found for this filter. Try another category or search term.</p>
                <?php endif; ?>
            </div>
        </div>
